...
merge_results <- function(results_dir,
                          ts,
                          model = c("D", "P", "NI", "S", "AR(1)", "AR(2)", "S1", "S1_2", "S1_3"),
  ...
  files <- file.path(results_dir, temp_result_filename(df$n, df$model, df$method_name))
  df2 <- tibble()
  walk(files, \(f) {
...
make_values_filter <- function(values) {
  function(data, column = model, .by = NULL, .preserve = FALSE) {
    data |>
